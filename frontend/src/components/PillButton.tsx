...
export default function PillButton({ buttonTitle, onClick, activeColor = "active:bg-indigo-900" }: Props) {
    ...
    return (
        <div className={`flex whitespace-nowrap justify-center items-center rounded-3xl text-base text-center px-2 h-7 cursor-pointer 
        bg-white ${activeColor} active:text-white text-black `}
            onClick={onClick}>
            {buttonTitle}
        </div>
    );
